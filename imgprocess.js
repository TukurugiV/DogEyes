...
function imgprocess(pixels, deuter, decbright, decac) {
...
async function processAndDrawImage(inputImagePath, canvas, deuter, decbright, decac) {
    const image = new Image();
    image.onload = async () => {
        const maxWidth = window.innerWidth * 0.9; // 最大幅
        ...
        document.getElementById('beImg').style.width = width + 'px';
        document.getElementById('beImg').style.height = height + 'px';
        document.getElementById('beImg').src = image.src;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const deuter = true; // deuteranopia効果を適用するかどうか
    const decbright = true; // "decreased brightness discrimination"効果を適用するかどうか
    const decac = 2.0; // 視覚鋭さを減少させる係数
    ...
    fileInput.addEventListener('change', function(e) {
        ...
        reader.onload = function(e) {
            processAndDrawImage(e.target.result, canvas, deuter, decbright, decac);
        }
